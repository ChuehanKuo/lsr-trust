    ...
    <Section
      ...
      actions={
        <a
          ...
          download
          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-ink-300 text-ink-700 text-[13px] font-medium hover:bg-ink-100 transition-colors"
        >
...
function ExtractionTemplate() {
  return (
    <div className="space-y-6">
      <StaggerChildren className="grid md:grid-cols-2 gap-3">
        {EXTRACTION_DOMAINS.map((domain, idx) => (
          <StaggerItem key={domain.domain}>
            <div className="p-4 bg-white rounded-lg border border-ink-200 hover:border-ink-300 transition-colors">
              <div className="flex items-baseline gap-3 mb-2">
                <span className="text-[11px] font-mono text-ink-400 tabular-nums">{String(idx + 1).padStart(2, '0')}</span>
                <h4 className="text-[14px] font-semibold text-ink-900">{domain.domain}</h4>
              </div>
              <div className="flex flex-wrap gap-1.5 ml-7">
                {domain.variables.map((v) => (
                  <span key={v} className="px-2 py-0.5 rounded bg-ink-100 text-[11px] text-ink-500">{v}</span>
                ))}
              </div>
            </div>
          </StaggerItem>
      ...
      <div className="p-5 bg-white rounded-lg border border-ink-200">
